...
function  isKnativeSupport(){
   cat ${cnfRoot}/data/k8s/nodes.ini | grep -v "#" | grep -w knative-eventing || return $?
   cat ${cnfRoot}/data/k8s/nodes.ini | grep -v "#" | grep -w knative-serving || return $?
}

function deployKnativeOperator(){

 local operatorYaml="/home/opuser/knative/knative-operator.yaml"
